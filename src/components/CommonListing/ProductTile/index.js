...
export default function ProductTile({ item }) {
  ...
  return (
    <div className="w-full max-w-xs p-4 mx-auto border-2 border-[#A02F58] rounded-lg group">
      <div
      ...
      >
        <div className="relative w-full mb-4 overflow-hidden h-52">
          {item.onSale == "yes" && (
            <div className="absolute top-0 left-0 z-10 px-2 py-1 mt-5 ml-4 text-xs text-white bg-[#A02F58] rounded-full">
              Sale
            </div>
          )}
          <img
            src={item.imageUrl}
            alt="product image"
            className="object-cover object-center w-full h-full transition-all duration-300 group-hover:scale-125"
          />
        </div>
        ...
        <div className="flex items-center gap-2">
          <IoLocationSharp className="w-4 h-4 text-[#A02F58]" />
