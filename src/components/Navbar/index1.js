  ...
  return (
    <div className="relative">
      {isAuthUser && isAdminView ? (
        ...
        >
          <FaPlus className="w-6 h-6 text-[#A02F58]" />
        </button>
      ) : null}
...
export default function Navbar() {
  ...
  return (
    <header className="flex items-center p-4 px-2 bg-white shadow-md justify-evenly">
      <div className="pb-1 text-2xl font-bold text-[#A02F58]">Elysian</div>
      <div className="flex items-center space-x-4">
        <input
          type="text"
          placeholder="Search in Elysian..."
          className="px-4 py-2 border rounded-full outline-none w-[50vw] focus:ring-1 focus:ring-[#A02F58]"
        />
        <NavItems
          ...
          router={router}
        />
        <FaShoppingCart className="text-2xl text-[#A02F58] cursor-pointer" />
        <FaHeart className="text-2xl text-[#A02F58] cursor-pointer" />

        {isAdminView ? (
          <button
            className="font-semibold text-white bg-[#A02F58] border rounded-full hover:text-[#A02F58] hover:bg-pink-100 md:px-4 md:py-2 lg:text-lg md:text-sm lg:px-4 lg:py-1"
            onClick={() => router.push("/")}
          >
            Client View
          </button>
        ...
        <div className="relative">
          {isAuthUser ? (
            <button
              ...
              className="flex items-center border border-[#A02F58] rounded-full hover:bg-pink-100"
            >
              <img
            ...
            <Fragment>
              <button
                className="px-4 py-1 font-semibold text-[#A02F58] border-2 border-[#A02F58] rounded-full hover:bg-pink-100"
